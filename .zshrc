...
plugins=(zsh-syntax-highlighting shortcuts colored-man-pages ubuntu)
...
if [[ -z $SSH_AGENT_PID ]]; then
  ...
  ssh-add /mnt/c/Users/Ryan/.ssh/github_rsa 2> /dev/null
fi

function battery_pct_prompt() {
  b=$(/mnt/c/Windows/System32/wbem/WMIC.exe path win32_battery get EstimatedChargeRemaining | head -n 2 | tail -n 1 | sed 's/\([0-9]*\).*/\1/')
  if [ $b -gt 50 ] ; then color='green'
  elif [ $b -gt 20 ] ; then color='yellow'
  else color='red'
  fi
